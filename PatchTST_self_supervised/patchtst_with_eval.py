...
class LogEvalLossCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, **kwargs):
        # Access the logs directly to get the loss
        metrics = kwargs.get("metrics", {})
        if "eval_loss" in metrics:
            print(f"Evaluation Loss: {metrics['eval_loss']}")
        else:
            # If eval_loss isn't in metrics, try to get it from logs
            logs = state.log_history
            for log in reversed(logs):
                if "eval_loss" in log:
                    print(f"Evaluation Loss: {log['eval_loss']}")
                    break

def compute_metrics(eval_pred):
    logits, labels = eval_pred
    loss = torch.nn.functional.mse_loss(torch.tensor(logits), torch.tensor(labels)).item()
...
def main():
    ...
    training_args = TrainingArguments(
        output_dir='/home/gayal/ssl-project/PatchTST/PatchTST_self_supervised/saved_models/test_run_10_recordings_eval/outputs', #"./checkpoint/patchtst-ego4d/pretrain/output/",
        ...
        eval_steps=2,
        # save_total_limit=3,
        logging_dir='/home/gayal/ssl-project/PatchTST/PatchTST_self_supervised/saved_models/test_run_10_recordings_eval/logs',#"./checkpoint/patchtst-ego4d/pretrain/logs/",  
